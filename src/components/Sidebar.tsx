...
export const Sidebar: React.FC = () => {
  ...
  // Strict Role-Aware Navigation Mapping (Requirement 6)
      ...
      case 'ISSUER':
        return {
          ...
          items: [
            ...
            { id: 'issuer-submit', label: 'Submit REC', path: '/issuer/submit', icon: PlusCircle, isAction: true },
            { id: 'issuer-my-recs', label: 'My RECs', path: '/issuer/my-recs', icon: FileSpreadsheet },
            { id: 'issuer-verification', label: 'Verification', path: '/issuer/verification', icon: BadgeCheck },
            { id: 'issuer-alerts', label: 'Risk Alerts', path: '/issuer/risk-alerts', icon: ShieldAlert },
      ...
      case 'BUYER':
        return {
          ...
          items: [
            ...
            { id: 'buyer-my-recs', label: 'My RECs', path: '/buyer/my-recs', icon: CheckCircle2 },
            { id: 'buyer-transactions', label: 'Transactions', path: '/buyer/transactions', icon: DollarSign },
            { id: 'buyer-verification', label: 'Verification', path: '/buyer/verification', icon: Award },
      ...
      case 'AUDITOR':
        return {
          ...
          items: [
            ...
            { id: 'auditor-recs', label: 'RECs', path: '/auditor/recs', icon: FileSpreadsheet },
  ...
  const handleNavClick = (item: SidebarNavItem) => {
    if (item.isAction && item.id === 'issuer-submit') {
      setIsAddRecOpen(true);
      setIsMobileMenuOpen(false);
      navigateToPath('/issuer/my-recs');
      return;
    }
          ...
          >
              ...
              {(canonicalRole === 'ADMIN' || canonicalRole === 'ISSUER') && (
                <div className="mb-3">
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-semibold shadow-md shadow-emerald-900/30 transition-all cursor-pointer min-h-[44px]"
                  >
                    <PlusCircle className="w-4 h-4" />
                    <span>+ Submit / Mint REC</span>
                  </button>
                </div>
              )}
            ...
            <div className="p-4 border-t border-slate-800 bg-slate-950/80">
              ...
              <p className="text-[11px] text-slate-400 font-mono">
                Role: <span className="text-emerald-300 font-semibold">{canonicalRole}</span>
              </p>
            </div>
          </div>
        </div>
          ...
          {(canonicalRole === 'ADMIN' || canonicalRole === 'ISSUER') && (
            <div className="mb-3 px-0.5">
              <button
                ...
                className={`flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-semibold shadow-md shadow-emerald-950/30 transition-all cursor-pointer ${
                  isSidebarCollapsed ? 'w-full py-2.5 px-0' : 'w-full py-2 px-3'
                }`}
                title="Create and register a new Renewable Energy Certificate"
              >
                <PlusCircle className="w-4 h-4 shrink-0" />
                {!isSidebarCollapsed && <span>+ Submit REC</span>}
              </button>
            </div>
          )}
